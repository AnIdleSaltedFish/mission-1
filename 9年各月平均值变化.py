...
# 遍历根目录下的所有文件夹和文件,,批量去除二级文件夹下txt文件的Y
...
for root, dirs, files in os.walk('C:/Users/28166/Desktop/环科院培训/编程任务一/Beijing'):  # 在beijing这个文件夹疯狂遍历
    ...
    for dir in dirs:    # dir是Y2013文件夹
        # 拼接二级文件夹路径
        dir_path = os.path.join(root, dir)  # 在这个级别，接下来要跑12个txt
        yeardata = []  # 放12个站点1年的数据，共144个数
        # 遍历二级文件夹下的所有文件
        ...
        # 定义新列表
        long_list = yeardata
        sub_lists = [long_list[i:i + 12] for i in range(0, len(long_list), 12)]   # sub_lists是二维列表，子列表存着每组的12个数（同一月份的十二个站点的值）
        average_list = [np.nanmean(sub_list) for sub_list in sub_lists]     # average_list
        print(average_list)
        totaldata.append(average_list)
print(totaldata)

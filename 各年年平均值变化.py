...
year = 2013
# 遍历根目录下的所有文件夹和文件
for root, dirs, files in os.walk('C:/Users/28166/Desktop/环科院培训/编程任务一/Beijing'):
    # 遍历二级文件夹
    for dir in dirs:
        ...
        yearmeanvalue = np.nanmean(last_list)
        yearmean.append(yearmeanvalue)
...
print(yearmean)
# 每个数保留两位小数
round_yearmean = np.round(yearmean,2)
print(round_yearmean)


x = [2013,2014,2015,2016,2017,2018,2019,2020,2021]
fig, ax = plt.subplots()
ax.plot(x, yearmean, 'o-')
plt.title('Trend of Annual Average of PM2.5', fontdict={'family': 'Times New Roman', 'size': 20, })
# 设置横纵坐标的名称以及对应字体格式
font2 = {'family': 'Times New Roman', 'weight': 'normal', 'size': 14, }
plt.xlabel('Year', font2)
plt.ylabel('Annual Average of PM2.5', font2)
# 横纵轴刻度一起设置
